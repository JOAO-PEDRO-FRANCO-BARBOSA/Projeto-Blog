...
* **Dark/Light Mode:** O projeto suporta os dois temas. Evita fixar cores inflexíveis (ex: `text-white` ou `bg-black`) a menos que o contraste exija que essa cor seja imutável em ambos os temas (ex: rodapés escuros). Utiliza sempre as variantes adaptativas do Tailwind (ex: `text-slate-900 dark:text-gray-100` ou `bg-white dark:bg-slate-900`).
